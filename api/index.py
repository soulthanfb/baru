...
# Pastikan URL dan KEY Anda benar
...
# Kita gabungkan CSS dan JS langsung di sini agar praktis satu file
HTML_TEMPLATE = """
<!DOCTYPE html>
<html lang="id">
<head>
...
<body>
    ...
    <div class="table-container">
        <h2 style="margin-top: 30px;">Riwayat Akses</h2>
        <table>
            <thead>
                <tr>
                    <th>Waktu</th>
            ...
            <tbody>
                {% for log in logs %}
                <tr>
                    <td>{{ log.waktu[:19].replace('T', ' ') }}</td>
                    <td><b>{{ log.nama }}</b></td>
                    <td><code>{{ log.uid }}</code></td>
                    <td>
                        <span class="badge {% if log.status == 'Masuk' %}bg-green{% else %}bg-red{% endif %}">
                            {{ log.status }}
                        </span>
                    </td>
                </tr>
    ...
    <script>
        // FUNGSI AUTO-FILL: Cek UID terbaru setiap 2 detik
        async function checkLatestScan() {
            try {
                ...
                // Jika ada data UID dan berbeda dengan yang sekarang, update
                if (data.uid && data.uid !== "BELUM ADA") {
                    // Cek apakah input field masih kosong atau placeholder, update jika ya
                    if(inputField.value !== data.uid) {
                        inputField.value = data.uid;
                        inputField.style.backgroundColor = "#e8f0fe"; // Efek kedip visual
        ...
        // Jalankan fungsi checkLatestScan setiap 2000ms (2 detik)
...
@app.route('/')
def home():
    # Ambil 10 log terakhir dari Supabase untuk ditampilkan di tabel
    logs = supabase.table("logs").select("*").order("id", desc=True).limit(10).execute()
    return render_template_string(HTML_TEMPLATE, logs=logs.data)
...
# Endpoint untuk JavaScript mengambil UID terakhir
@app.route('/api/last-scan', methods=['GET'])
def get_last_scan():
    # Ambil data dari tabel temp_scan baris ID=1
    scan = supabase.table("temp_scan").select("*").eq("id", 1).execute()
    if scan.data:
        return jsonify(scan.data[0])
    return jsonify({"uid": "BELUM ADA"})

# Endpoint Utama yang dipanggil Wemos
@app.route('/api/akses', methods=['POST'])
def cek_akses():
    data = request.json
    uid_kartu = data.get('uid')
    
    # 1. SELALU UPDATE 'temp_scan' (untuk fitur Auto-fill)
    # Kita update baris dengan ID=1
    try:
    ...
    if not user_query.data:
        # Jika tidak terdaftar, kembalikan 403 tapi UID sudah tersimpan di temp_scan
